...
data_daily_confirmed=table2array(data(6,4:end));
%remove all NaN values from array
data_daily_confirmed=data_daily_confirmed(~isnan(data_daily_confirmed));
...
data_daily_confirmed=data_daily_confirmed(72:258);
Y=data_daily_confirmed;
X=1:1:length(data_daily_confirmed);
...
dist=fitdist(X,'Nakagami','frequency',data_daily_confirmed);
yFitted1=pdf(dist,X);
...
bar(Y);
%errors computed , then RMSE(error=normalized original values-values gained from pdf)
errors=Y-yFitted1;
rmse(1,1)=sqrt(1/(length(data_daily_confirmed)-2)*sum(errors.^2) );
...
dist=fitdist(X,'GeneralizedExtremeValue','frequency',data_daily_confirmed);
yFitted2=pdf(dist,X);
...
dist=fitdist(X,'Rayleigh','frequency',data_daily_confirmed);
yFitted3=pdf(dist,X);
...
dist=fitdist(X,'Rician','frequency',data_daily_confirmed);
yFitted4=pdf(dist,X);
...
dist=fitdist(X,'Normal','frequency',data_daily_confirmed);
yFitted5=pdf(dist,X);
...
legend('Bargraph','Nakagami model','GeneralizedExtremeValue model','Rayleigh model','Rician model','Normal model');
title('Bargraph of confirmed cases for Armenia during first wave hit')
%find best model's index(minimum RMSE=best model)
best_model=find(Rsq==max(Rsq));
message_console=sprintf('Best model is the %s model distribution with an R squared of %4.4f',distributions{best_model},Rsq(best_model));
 ...
 %fitted one for our daily cases distribution. RMSE values are also
 %computed for a more complete picture of the model's perfomance. Whether
 %the same distribution could be used to describe the daily deaths
 %distribution as well is commented on the second script for this exercise.